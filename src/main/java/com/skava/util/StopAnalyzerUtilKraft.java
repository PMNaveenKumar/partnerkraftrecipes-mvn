    ...
    public static List<String> parseKeywords(String stopWords, String keywords, boolean isNonEngCharSearch, Campaign campaign, String region, boolean skipStopWordCheck) throws ServerException
    {
        ...
        if (keywordsArr != null)
        {
            for (int i = 0; i < keywordsArr.length; i++)
            {
                ...
                if (skipStopWordCheck)
                {
                    result.add(word);
                }
                else if (word.length() > 0 && !stopWordList.contains(word))
                {
                    result.add(word);
                }
            }
