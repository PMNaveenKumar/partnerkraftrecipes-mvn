...
var kraftUserRecipes = new ApplePie({
	url 	: 	'http://api.kraftapps.com/v2/Community/UserRecipes',
	get 	: 	{},
	rest:['userRecipeId']
});

var wellioUserRecipes = new ApplePie({
	url 	: 	'http://kaiseki.endpoints.wellio-integration.cloud.goog/api/recipes/?key=AIzaSyB7HKCPVdsGOkn46LlzOHUpLqzo2GKcdVc',
	get 	: 	{},
	headers : {
...
var wellioGetUserRecipes = new ApplePie({
	url 	: 	'http://kaiseki.endpoints.wellio-integration.cloud.goog/api/recipes',
	get 	: 	{
		key:'AIzaSyB7HKCPVdsGOkn46LlzOHUpLqzo2GKcdVc'
	},
	rest:['wellioReferenceId']
});


function getWellioGetUserRecipes(config, callback) {
	wellioGetUserRecipes.send({
		rest:{
			wellioReferenceId: config.wellioReferenceId
		}
	}, callback);
}

function getKraftUserRecipes(config, callback) {
	kraftUserRecipes.send({
		rest:{
			userRecipeId: config.userRecipeId
		}
	}, callback);
}
 function getWellioUserRecipes(config,callback){
 	wellioUserRecipes.send({
		post:{
			post:config.post
			}
 	},callback)
		...
		KraftRecipe.send({
			...
			try {
				...
				if(e.Recipes){
					...
					if (subset.length) {
						// console.log('***** Missing Recipes *****', true);
...
module.exports = {
	...
	getKraftUserRecipes:getKraftUserRecipes,
	getWellioUserRecipes:getWellioUserRecipes,
	getWellioGetUserRecipes: getWellioGetUserRecipes
}